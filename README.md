...
dataset, so the demo path always works. Contact emails are never generated by
OpenAI or trusted from fallback data: Fiber-returned emails are preserved when
valid, and Apollo Bulk People Enrichment is used to verify/enrich the final
five suggested contacts when `APOLLO_API_KEY` is configured.
...
- `/api/v1/narrow` returns at most 5 people and strips fallback emails unless
  Fiber or Apollo provides the address. Apollo enrichment uses
  `POST /api/v1/people/bulk_match` via the documented `/api/v1` base URL.
